...
from cleverhans.serial import PicklableGetVariable as PGV
...
class Linear(Layer):
  ...
  def set_input_shape(self, input_shape):
    ...
    self.W = PGV(init, "W", self.scope)
    if self.use_bias:
      self.b = PGV((np.zeros((self.num_hid,))
                    + self.init_b).astype('float32'), "b", self.scope)
